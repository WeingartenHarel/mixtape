...
export default {
	...
	computed: { 
		mix() {
			...
			if (this.mixId) {
				//console.log('getting mix song list')
		...
		mixSongs(){
			...
			if (this.$route.params.mixId) {
				...
				console.log('getting new mix song list')
					var mix = this.$store.getters.getEmptyMix
					if(!mix) return
					this.mixCopy = JSON.parse(JSON.stringify(mix));
					//console.log('this.mixCopy',this.mixCopy)
					return this.mixCopy.songs.filter(song => {
	...
	created() {
		...
		console.log('songs this.mixId =',this.$route.params.mixId, this.$route.params )
	},
	components:{
		mixApiSearch,
		draggable,
	...
	watch:{
		'$route.params.mixId': function (mixId) {
			console.log('ing mixId',mixId,this.$route.params.mixId)
			if(this.$route.params.mixId){
				this.mixId = this.$route.params.mixId
				console.log('watch mixId',mixId,this.$route.params.mixId)
				this.$store.getters.getMix
			}else{
				this.mixId = null
				console.log('watch no mixId',mixId,this.$route.params.mixId)
				this.$store.getters.getMix
			}
		}
